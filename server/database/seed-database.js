...
const writeReservations = fs.createWriteStream('./server/database/reservations.csv');
...
function generatelisting(id, id_users) {
  ...
  let minimum_stay = Math.round(Math.pow(Math.random(), 3) * maxminimumstay) + 1;
  let base_price = faker.random.number({min: 10, max: 200});
...
function generatereservations(listing, resindexstart, rescount) {
  startdate = new Date();
  startdate.setDate(startdate.getDate() + faker.random.number(5));
...
function generatesinglereservation(id, listing) {
  ...
  let additionaldays = Math.round(30 * Math.pow(Math.random(), 3));
  
  let end_date = new Date(year, month, day + listing.minimum_stay + additionaldays);
  startdate = new Date(end_date.getFullYear(), end_date.getMonth(), end_date.getDate() + faker.random.number({min: 1, max: 5}));
  let adult_count = 1 + Math.round(4 * Math.pow(Math.random(), 1.5));
  let children_count = Math.round(5 * Math.pow(Math.random(), 2));
  let infant_count = Math.round(5 * Math.pow(Math.random(), 3));
  let user_id = faker.random.number(maxusers);
  ...
  // console.log(parseFloat(listing.cleaning_fee) + parseFloat(listing.service_fee));
  // console.log('pricebeforefees' + pricebeforefees);
...
function writeTenMillionListings(writeListings, writeReservations, encoding, callback) {
  ...
  writeReservations.write('id, id_listings, id_users, start_date, end_date, adult_count, children_count, infant_count, total\n', 'utf8');
  ...
  function write() {
    ...
    do {
      ...
      reservations.forEach((reservation)=>{
        let data2 = `${reservation.id},${reservation.listing_id},${reservation.user_id},${reservation.start_date},${reservation.end_date},${reservation.adult_count},${reservation.children_count},${reservation.infant_count},${reservation.total}\n`;
